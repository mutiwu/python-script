...
status = commands.getstatusoutput("easy_install pexpect")[0]
if status:
    print "the pexpect is not installed"
    sys.exit(1)
import pexpect

def driver_reload(prses, driver_name, timeout):
    time.sleep(3)
    stattime = time.time()
    while True:
        ...
        if not ( pid[0] and pid[1] ):
            break
        cmd_e = "modprobe %s" % driver_name
        cmd_d = "modprobe -r %s" % driver_name
        cmd_c = "lsmod |grep %s" % driver_name
        status, result = commands.getstatusoutput(cmd_c)
        if status:
            print "can not use lsmod, please check"
            sys.exit(1)
        if result:
            status = commands.getstatusoutput(cmd_d)
            if status:
                print "can not modprobe -r %s" % driver_name
                sys.exit(1)
        status = commands.getstatusoutput(cmd_e)
        if status:
            print "can not modprobe %s" % driver_name
            sys.exit(1)
        time.sleep(2)
        endtime = time.time()
